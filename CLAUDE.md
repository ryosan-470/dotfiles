...
- **Deny(常に拒否)**: `Bash(curl *)`, `Bash(wget *)` (任意のネットワークリクエスト防止)
- Note: `find` は Claude Code の組み込み read-only コマンドのため deny 対象外。`find -exec`/`find -delete` のような危険な形式は Claude Code が常にプロンプトを表示する仕様
