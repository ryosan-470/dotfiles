...
- 既存ファイルへの `Write` は必ず Read 済みの状態で行うこと。未 Read のまま Write すると "File has not been read yet" で失敗する。
- 同じファイルに続けて2回目の Edit をする前には再度 Read すること。Go リポジトリの多くは Edit/Write 直後に `make fmt` 等の自動整形を走らせる PostToolUse hook を持つ (例: gosimports/gofumpt) ため、1回目の Edit 後に裏で整形され、記憶している `old_string` が一致しなくなる。
- `Edit` の `old_string` には、コメント区切り行 (`// -----`) や定型の doc コメントのような、ファイル内で一意でない文字列をアンカーに使わないこと。前後の一意な行まで含めて範囲を広げる。
- 同一ファイルへの `Edit` を同一メッセージ内で並列発行しないこと。1件ずつ直列に実行する — 並列だと後続の Edit が古い内容を前提にして失敗する。
